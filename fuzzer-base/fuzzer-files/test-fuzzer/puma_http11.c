...
VALUE httpparse(VALUE parser)
{
  ...
  VALUE instr = rb_str_new_cstr(testcase);
  VALUE arg3 = INT2NUM(0);
  VALUE result;
  result = rb_funcall(parser, rb_intern("execute"), 3, rb_hash_new(), instr, arg3);
  return parser;
}

VALUE print_parse_catch(VALUE anything)
{
  rb_p(rb_str_new_cstr("Caught HttpParserError"));
  return anything;
}
...
int main(int argc, char** argv)
{
 ...
 // while(__AFL_LOOP(1000)) {
    ...
    VALUE parser = rb_class_new_instance(0, 0, cHttpParser);
    VALUE result = rb_rescue2(httpparse, parser, print_parse_catch, parser, eHttpParserError, 0);;
    VALUE result2 = rb_funcall(parser, rb_intern("finished?"), 0, Qnil);
    rb_p(result2);
    parser = Qnil;
    result = Qnil;
    result2 = Qnil;
